function [ data, g, data0, u_opt_array, d_opt_array ] = SafeControl(accuracy)
...
% data = shapeComplement(shapeRectangleByCenter(g, [0;0], [5;0.75*pi]));
data = shapeComplement(shapeSphere(g, [ 0; 0 ], targetRadius));
...
% disp(size(data))
figure()
contour(g.xs{1}, g.xs{2}, data,1)

endTime = cputime;
fprintf('Total execution time %g seconds\n', endTime - startTime);

simulation(g, u_opt_array, d_opt_array, data)
...
states = [X1(:) X2(:)];
u_opt_array = u_opt_array{end}(:);
d_opt_array = d_opt_array{end}(:);
