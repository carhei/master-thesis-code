...
pendulum.params.noise = 1;%+0.1*randn(x1_steps*x2_steps,1);            % standard deviation input disturbance
...
d0 = 10.0;                                %conservative disturbance estimate
init = 0.5*pi;
...
not_a = zeros(size(mdp.S,1), size(mdp.A,1));
A =  zeros(size(mdp.S,1), size(mdp.A,1));
%====================================
...
while count <=maxcount
    ...
    for n=1:steps-1
        [~, action] = max(Q(state,:)+A(state,:));
        A(state,:) = A(state,:) + 1./(1+not_a(state,:));
        A(state,action) = 0;
        not_a(state,:) = not_a(state,:)+1;
        not_a(state,action) = 0;
