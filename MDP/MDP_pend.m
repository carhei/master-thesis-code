clear
close all 
clc


pendulum.nstates = 2;
pendulum.params.g = 9.81;       % gravity constant
pendulum.params.m = 1;          % mass of pendulum
pendulum.params.l = 1;          % length of pendulum
pendulum.params.b = 0.2;        % damping coefficient
% pendulum.params.period = 2*pi*sqrt(pendulum.params.l/pendulum.params.g);
pendulum.params.h = 0.2;%01*pendulum.params.period;
pendulum.params.std = 0.1;



% MDP grid params
x1_bounds = [-pi/4, pi/4];
x1_steps = 21;
x2_bounds = [-1,1];
x2_steps = 21;
pendulum.grid.state_bounds = [x1_bounds; x2_bounds];
pendulum.grid.state_steps = [x1_steps; x2_steps];

pendulum.grid.input_bounds = [-10; 10];
pendulum.grid.input_steps =  9;

% MDP reward params
pendulum.reward = [-10, -1];
pendulum.edge_reward = [-20; -20];
pendulum.edge_width = 0.1;

pendulum.input_reward = -0.5;

% Build the MDP model

mdp = BuildMDP(pendulum);


% Solve the MDP model

opt_params.lambda = 0.99;
opt_params.epsilon = 0.0001;

[policy, n, vec0] = valueiteration(mdp, opt_params);


marker = 90*ones(size(policy));

figure(1); clf; colormap('jet')
scatter3(mdp.S(:,1), mdp.S(:,2), policy, marker, policy, 'filled');
xlabel('angle (rad)')
ylabel('rate (rad/s)')
zlabel('torque (Nm)')
title('Optimal Policy')
view(2)
axis equal

figure(2); clf; colormap('jet')
scatter3(mdp.S(:,1), mdp.S(:,2),abs((vec0(:,end)).^(1/4)), marker, abs((vec0(:,end)).^(1/4)), 'filled');
xlabel('angle (rad)')
ylabel('rate (rad/s)')
zlabel('value ')
title('Value Function')
view(2)
axis equal
